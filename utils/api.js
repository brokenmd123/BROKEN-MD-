  ...
  getTwitterDownload: async (url) => {
    try {
      ...
      const apis = [
        ...
        `https://twdown.net/api.php?url=${encodeURIComponent(url)}`,
        `https://twitter-downloader-api.com/api?url=${encodeURIComponent(url)}`
      ];
      ...
      for (const apiUrl of apis) {
        try {
          ...
          if (response.data) {
            ...
            if (response.data.videoUrl || response.data.video_url) {
              return {
                ...
                username: response.data.author || response.data.username || 'Unknown'
              };
            } else if (response.data.media && response.data.media[0]) {
              return {
